...
def calibratecamera(foldername):
    ...
    calibration_images = sorted(glob.glob(foldername))
    ...
    ret, K, dist, rvecs, tvecs = cv2.calibrateCamera(
        objectpoints, imagepoints, img.shape[::-1], None, None)
...
def save_camera_props(camera, degree, distancex, distancey):
    camera_props = []
...
def main():
    ...
    save_camera_props(cameras,degree,distancex,distancey)
